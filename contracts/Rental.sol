...
import "./interfaces/INtvrkRental.sol";

contract NtvrkRental is
    INtvrkRental,
    ERC721Holder,
    ERC1155Receiver,
    ERC1155Holder
{
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Rental::not admin");
        _;
    }
    ...
    function bundleCall(function(CallData memory) _handler, CallData memory _cd)
        private
    {
        require(_cd.nfts.length > 0, "Rental::no nfts");
    ...
    function distributePayments(
        ...
        require(
            ...
            "Rental::total payment wo collateral is zero"
        );
        require(sendLenderAmt > 0, "Rental::lender payment is zero");
        ...
        private
    {
        ...
        require(maxRentPayment > 0, "Rental::collateral plus rent is zero");
    ...
    function safeTransfer(
        ...
        if (is721(_cd.nfts[_cd.left])) {
            ...
            revert("Rental::unsupported token type");
        }
    }
    ...
    function handleRent(CallData memory _cd) private {
        ...
        for (uint256 i = _cd.left; i < _cd.right; i++) {
...

            {
                ...
                require(rentPrice > 0, "Rental::rent price is zero");
                require(nftPrice > 0, "Rental::nft price is zero");
    ...
    function ensureIsNotZeroAddr(address _addr) private pure {
        require(_addr != address(0), "Rental::zero address");
    }

    function ensureIsZeroAddr(address _addr) private pure {
        require(_addr == address(0), "Rental::not a zero address");
    }

    function ensureIsNull(Lending memory _lending) private pure {
        ensureIsZeroAddr(_lending.lenderAddress);
        require(_lending.maxRentDuration == 0, "Rental::duration not zero");
        require(_lending.dailyRentPrice == 0, "Rental::rent price not zero");
        require(_lending.nftPrice == 0, "Rental::nft price not zero");
    }

    function ensureIsNotNull(Lending memory _lending) private pure {
        ensureIsNotZeroAddr(_lending.lenderAddress);
        require(_lending.maxRentDuration != 0, "Rental::duration zero");
        require(_lending.dailyRentPrice != 0, "Rental::rent price is zero");
        require(_lending.nftPrice != 0, "Rental::nft price is zero");
    }

    function ensureIsNull(Renting memory _renting) private pure {
        ensureIsZeroAddr(_renting.renterAddress);
        require(_renting.rentDuration == 0, "Rental::duration not zero");
        require(_renting.rentedAt == 0, "Rental::rented at not zero");
    }

    function ensureIsNotNull(Renting memory _renting) private pure {
        ensureIsNotZeroAddr(_renting.renterAddress);
        require(_renting.rentDuration != 0, "Rental::duration is zero");
        require(_renting.rentedAt != 0, "Rental::rented at is zero");
    }

    function ensureIsLendable(CallData memory _cd, uint256 _i) private pure {
        require(_cd.lentAmounts[_i] > 0, "Rental::lend amount is zero");
        require(_cd.lentAmounts[_i] <= type(uint8).max, "Rental::not uint8");
        require(_cd.maxRentDurations[_i] > 0, "Rental::duration is zero");
        require(
        ...
        require(
            ...
            "Rental::rent price is zero"
        );
        require(uint32(_cd.nftPrices[_i]) > 0, "Rental::nft price is zero");
    }
    ...
    function ensureIsRentable(
        ...
        require(
            ...
            "Rental::cant rent own nft"
        );
        require(_cd.rentDurations[_i] <= type(uint8).max, "Rental::not uint8");
        require(_cd.rentDurations[_i] > 0, "Rental::duration is zero");
        require(
            _cd.rentDurations[_i] <= _lending.maxRentDuration,
            "Rental::rent duration exceeds allowed max"
        );
    }
    ...
    function ensureIsReturnable(
        ...
        require(_renting.renterAddress == _msgSender, "Rental::not renter");
        require(
    ...
    function ensureIsStoppable(Lending memory _lending, address _msgSender)
        ...
        pure
    {
        require(_lending.lenderAddress == _msgSender, "Rental::not lender");
    }
    ...
    function ensureIsUnpackablePrice(bytes4 _price, uint256 _scale)
        ...
        pure
    {
        require(uint32(_price) > 0, "Rental::invalid price");
        require(_scale >= 10000, "Rental::invalid scale");
    }

    function ensureTokenNotSentinel(uint8 _paymentIx) private pure {
        require(_paymentIx > 0, "Rental::token is sentinel");
    }
    ...
    function isPastReturnDate(Renting memory _renting, uint256 _now)
        ...
        returns (bool)
    {
        require(_now > _renting.rentedAt, "Rental::now before rented");
        return
    ...
    function setRentFee(uint256 _rentFee) external onlyAdmin {
        require(_rentFee < 10000, "Rental::fee exceeds 100pct");
        rentFee = _rentFee;
    }
